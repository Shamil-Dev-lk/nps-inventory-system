...
export function BarcodeGeneratorDialog({ items, open, onOpenChange }: BarcodeGeneratorProps) {
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 grid w-full max-w-2xl translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg sm:rounded-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 py-4">
            ...
            <div className="md:col-span-3 bg-muted/30 rounded-xl border border-dashed border-border/50 p-4 overflow-auto max-h-[50vh] custom-scrollbar">
              <div ref={printRef} className="bg-white p-4 w-full flex flex-col gap-4 items-center">
                {items.length === 0 ? (
                  <p className="text-muted-foreground my-10">No items selected.</p>
                ) : (
                  items.map((item, idx) => (
                    <div key={idx} className="flex flex-col items-center border border-gray-200 p-3 rounded-lg text-black page-break-inside-avoid w-64 bg-white shadow-sm">
                      <div className="flex items-center gap-2 mb-2 w-full border-b pb-2 justify-center text-center">
