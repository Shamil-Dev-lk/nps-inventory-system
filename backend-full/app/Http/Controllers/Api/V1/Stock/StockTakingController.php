...
class StockTakingController extends Controller {
    ...
    public function approve(StockTaking $stockTaking, \App\Services\StockLedgerService $ledgerService): JsonResponse {
        $this->authorize("approve-stock-taking");
        
        DB::beginTransaction();
        try {
            foreach ($stockTaking->items as $stItem) {
                if ($stItem->variance != 0) {
                    $item = $stItem->item;
                    $warehouse = $stockTaking->warehouse;
                    $qtyIn = $stItem->variance > 0 ? $stItem->variance : 0;
                    $qtyOut = $stItem->variance < 0 ? abs($stItem->variance) : 0;
                    $type = $stItem->variance > 0 ? 'adjustment_increase' : 'adjustment_decrease';
                    
                    $ledgerService->record($item, $warehouse, $type, $stockTaking->st_number, "StockTaking", $stockTaking->id, $qtyIn, $qtyOut, $stItem->unit_cost ?? $item->average_cost, $stItem->variance_reason ?? 'Stock Taking Variance');
                }
            }
            ...
            return response()->json(["message"=>"Stock taking approved and stock levels adjusted."]);
        } catch(\Exception $e) { 
            DB::rollBack(); 
